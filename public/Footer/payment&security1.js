console.log("payment&security.js");
...
const aboutForm = (e) => {
  ...
  const submitData = async (submitData) => {
    let aboutRef = db.collection("footer").doc("paymentSecurity");
  ...
  submitData(data).then(async () => {
    if (IMAGE) {
      await storageService
        .ref(`footer/paymentSecurity/${IMAGE.name}`)
        .put(IMAGE);
      let imgUrl;
      await storageService
        .ref(`footer/paymentSecurity/${IMAGE.name}`)
      ...
      aboutRef.get().then(async (doc) => {
        ...
        if (IMAGE) {
          ...
          storageService
            .ref(`footer/paymentSecurity/${docData.img}`)
            .delete()
            .then(async () => {
