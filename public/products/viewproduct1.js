    ...
    await db
        ...
        .then((snapshot) => {
            ...
            snapshotDocs.map((doc) => {
                let docData = doc.data();
                options += `
      <option ${docData.name == data ? "selected" : null}  value="${doc.id}__${docData.name
                    }">${docData.name}</option>
      `;
            });
        ...
        .then((snapshot) => {
            ...
            doc.subCategory.map((sc) => {
                if (+sc.id === +scId) {
                    console.log(docId);
                    options += `
          <option  selected value="${docId}__${sc.id}__${sc.name}">${sc.name
                        }</option>
          `;
                } else {
                    options += `
    ...
    await db
        ...
        .then((snapshot) => {
            ...
            snapshotDocs.map((doc) => {
                ...
                options += `
          <h5 class="addon"><input ${checkboxSelect ? "checked" : null
                    } type="checkbox" name="product-addon" value="${doc.id}__${docData.name
