      ...
      `;
        console.log("REETIKKKKKKKKKKKKKKKKKKKKKKKKKKK")
          ...
          try {
            ...
            // cron.schedule('0 4 * * *', () => {
            transporter.sendMail(mailOptions);
            console.log("email sent to:", doc.data().Email);
            transporter.close();
            // await admin.firestore().collection("Customers").doc(newValue.userId).update(o.isEmailSent,true)
            let statusRef = await admin.firestore().collection("Customers").doc(doc.data().userId);
            ...
            console.error(
              "There was an error while sending the email:" + doc.data().Email,
              error
            );
          }
