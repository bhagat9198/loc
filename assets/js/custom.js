...
// CANCEL CLICK ENDS
    ...
    $(document).on("click", "#qaddcrt" , function(){
      var qty = $('.qttotal').html();
      var pid = $(this).parent().parent().parent().parent().parent().parent().parent().parent().parent().find("#product_id").val();
      var message_cart = $(".message_cart").val();
      ...
      window.location = mainurl+"/addtonumcart?id="+pid+"&qty="+qty+"&size="+sizes+"&color="+colors.substring(1, colors.length)+"&size_qty="+size_qty+"&size_price="+size_price+"&size_key="+size_key+"&keys="+keys+"&values="+values+"&prices="+prices+"&message="+message_cart;

     });
     
     
      $(document).off('click', '.removeaddon ').on('click', '.removeaddon ', function(){
      var id = $(this).data('id');
        ...
        var token = $('meta[name="csrf-token"]').attr('content');
         ...
         $('.add_addons:checked').each(function() {
            ...
            $.ajax({
                    ...
                    data:{ids:JSON.stringify(selectedLanguage)},
                    dataType:"json",
                    headers: {
...
// Add By ONE
      ...
      $(document).off("click", ".adding").on("click", ".adding" , function(){
         ...
         $("#qty"+itemid).html(qty);
            ...
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        $(".cart-total").html("₹"+(Number(data[0].slice(1))+Number(addons_price)));
                       $(".main-total").html("₹"+(Number(data[3].slice(1))+Number(addons_price)));
                        // alert(Number(data[3].slice(1))+Number(addons_price));
...
// Reduce By ONE
      ...
      $(document).off("click", ".reducing").on("click", ".reducing" , function(){
        var pid =  $(this).parent().parent().find('.prodid').val();
        var itemid =  $(this).parent().parent().find('.itemid').val();
        var size_qty = $(this).parent().parent().find('.size_qty').val();
         if(Number(qty)==1){
       ...
       $(document).off("click", ".addingg").on("click", ".addingg" , function(){
        ...
        var $t = $(this);
            ...
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        {
                        $(".discount").html($("#d-val").val()); 
                        $(".cart-total").html("₹"+(Number(cart_total.slice(1))+Number(data.main_amount)));
                       $(".main-total").html("₹"+(Number(main_total.slice(1))+Number(data.main_amount)));
                       $t.closest('tr').find('.qttotal1').text(data.qty);
                       $t.closest('tr').find('.total-price p').text("₹"+(Number(data.qty)*Number(data.main_amount)));
                        // alert(Number(data[3].slice(1))+Number(addons_price));
                        // $("#prc"+itemid).html(data[2]);
                        // $("#prct"+itemid).html(data[2]);
                        // $("#cqt"+itemid).html(data[1]);
...
// Reduce By ONE
      ...
      $(document).off("click", ".reducingg").on("click", ".reducingg" , function(){
        var aid = $(this).data('id');
        var qty = $(this).closest('ul').find('.qttotal1').text();
        if(Number(qty)==1){
            return false;
        ...
        // console.log(addonss_price);
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        {
                            $("#coupon_form").toggle();
                            // $(".discount").html("₹"+(Number(data[4].slice(1))+Number(addonss_price)));OFF25
                            $(".main-total").html("₹"+(Number(data[0].slice(1))+Number(addonss_price)));
...
// Review Section Ends
    function get_offer_coupon()
    {
       $.ajax({
       method:"get",
       url:mainurl+"/get_coupon_offer",
       datatype:"json",
       success:function(data)
       {
           var data = JSON.parse(data);
           $("#coupon_offerid").html(data['title']);
       }
      });   
    }
